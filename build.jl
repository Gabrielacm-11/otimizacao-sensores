...
# ╔═╡ ff73ace5-63e3-4f29-b1c7-f6c0c6d70687
...
function f(n)
	...
	Aᵢ = x
	for i = 1:n
		aux = ((xₖ.-x[i])./norm(xₖ.-x[i])).-((xₖ.-x[i])./norm(xₖ.-x[i]))
		Aᵢ[i][1]=aux[1]
		Aᵢ[i][2]=aux[2]
	end
	return Aᵢ
...
# ╔═╡ a317b6c0-b17e-45e4-a1ec-28d57e79ad7a
f(9)
...
begin
	...
	@variable(IC,xA>=0)
	@variable(IC,xA>=0)
 	@objective(IC, Min, A*x+C-s)
 	@constraint(IC, A*x+C-s <= 0)
 	print(IC)
 	optimize!(IC)
